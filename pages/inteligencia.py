...
.diag-pergunta-txt { font-size:1.0rem; font-weight:600; color:#071F12; line-height:1.5; }
.stButton > button { background:#FFFFFF !important; border:1.5px solid #C8E6D0 !important; border-radius:10px !important; color:#071F12 !important; font-size:0.85rem !important; font-weight:600 !important; padding:10px 16px !important; width:100% !important; text-align:left !important; }
.stButton > button:hover { background:#F4FBF6 !important; border-color:#1A6B3A !important; }
...
def get_arvore_node(status, curva):
    s = str(status).strip().upper(); c = str(curva).strip().upper()
    tree = ARVORE.get(s, {})
...
with c2:
    ...
    else: sel_cid='Todas'; st.selectbox("CIDADE",['Todas'],disabled=True)
...
for fl,nl,bg,fg,fn in FAIXAS_EROSAO:
    n_f=int(df_f[df_f['_EROSAO'].apply(fn)].shape[0]); pct_f=round(n_f/n_total*100,1) if n_total>0 else 0.0
...
r_nome=str(row.get(col_cli,''));    r_vend=str(row.get(col_vend,'')) if col_vend else '—'
...
<div class='missao-card'>
  ...
  <div class='missao-objetivo'><div class='missao-obj-lbl'>POR QUE VOCÊ ESTÁ LIGANDO</div><div class='missao-obj-txt'>{miss.get('objetivo','—')}</div></div>
  <div class='missao-callout'><div class='missao-call-lbl'>O QUE VOCÊ PRECISA FAZER</div><div class='missao-call-txt'>{miss.get('missao','—')}</div></div>
  <div class='missao-metrics'>
    ...
    <div class='missao-metric'><div class='missao-metric-lbl'>QUANDO LIGAR DE NOVO</div><div class='missao-metric-val'>{miss.get('reavaliacao','—')}</div></div>
  </div>
  <div class='missao-sucesso'><div class='missao-suc-ico'>&#10003;</div><div><div class='missao-suc-lbl'>COMO SABER QUE DEU CERTO</div><div class='missao-suc-txt'>{miss.get('sucesso','—')}</div></div></div>
...
if abord:
    st.markdown("<div style='margin-top:16px;'>",unsafe_allow_html=True)
    st.markdown(f"<div class='abordagem-bloco'><div class='abordagem-lbl'>COMO COMEÇAR A CONVERSA</div><div class='abordagem-txt'>{abord}</div></div>",unsafe_allow_html=True)
    st.markdown("</div>",unsafe_allow_html=True)
...
if node is None:
    st.info("Árvore de diagnóstico não disponível para esta combinação de status e curva."); st.stop()

if historico:
    hist_html="".join([f"<div class='diag-hist-item'><div class='diag-hist-p'>{h['p']}</div><div class='diag-hist-r'>→ {h['r']}</div></div>" for h in historico])
...
if node['tipo']=='pergunta':
    ...
    st.markdown(f"<div class='diag-pergunta-card'><div class='diag-step'>PERGUNTA {step}</div><div class='diag-pergunta-txt'>{node['texto']}</div></div>",unsafe_allow_html=True)
    ...
    escala_html=(
        f"<div class='diag-res-escala'><div class='diag-res-escala-ico'>!</div>"
        f"<div><div class='diag-res-escala-lbl'>ACIONE O GESTOR</div>"
        f"<div class='diag-res-escala-txt'>Este diagnóstico requer escalonamento imediato.</div></div></div>"
    ) if escalar else (
        f"<div class='diag-res-ok'><div class='diag-res-ok-ico'>&#10003;</div>"
        f"<div class='diag-res-ok-txt'>Gestão direta pelo vendedor — sem necessidade de escalonar.</div></div>"
    )
    st.markdown(f"""
<div class='diag-resultado'>
  <div class='diag-res-header'><div class='diag-res-header-txt'>DIAGNÓSTICO CONCLUÍDO</div></div>
  <div class='diag-res-nome'><div class='diag-res-nome-lbl'>DIAGNÓSTICO</div><div class='diag-res-nome-val'>{node['nome']}</div></div>
  <div class='diag-res-acao'><div class='diag-res-acao-lbl'>O QUE FAZER AGORA</div><div class='diag-res-acao-txt'>{node['acao']}</div></div>
  <div class='diag-res-prazo'><div class='diag-res-prazo-lbl'>PRAZO — CURVA {r_curv}</div><div class='diag-res-prazo-val'>{prazo_diag}</div></div>
  {escala_html}
</div>
""",unsafe_allow_html=True)
