...
def engine_star(row, lp, cp):
    ...
    if cp < (lp * 0.80):
        return "🚨 QUEDA ACENTUADA", lp, (
            ...
            "Houve alguma mudança no seu processo ou mercado que eu precise saber para te ajudar?'"
        )
    if cp < (lp * 0.95): 
        return "🔴 QUEDA", lp, (
    ...
    if cp > (lp * 1.05): 
        return "🟢 CRESCIMENTO", int(cp * 1.05), (
            ...
            "precisamos ajustar seu mix. Vamos planejar o próximo mês?'"
        )
    return "🔵 ESTÁVEL", int(lp * 1.05), (
        ...
        "podemos confirmar a reposição? Há algum novo desafio de margem hoje?'"
    )
