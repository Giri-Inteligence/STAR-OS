...
if up:
    ...
    if col_meses:
        ...
        df_proc['TOTAL LP'] = df_proc[col_ativas].sum(axis=1).astype(int)
        df_proc['MÉDIA LP'] = (df_proc[col_ativas].mean(axis=1)).astype(int)
        df_proc['MÉDIA CP'] = (df_proc[col_ativas[-3:]].mean(axis=1)).astype(int)
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as wr:
            ...
            # DESTAQUE TOTAL LP (CINZA E NEGRITO)
            ...
            for r_idx, row in df_proc.iterrows():
                for c_idx, col_name in enumerate(ordem):
                    ...
                    if col_name == 'STATUS':
                        ...
                        ws.write_string(r_idx + 1, c_idx, str(val), f)
                    elif col_name == 'TOTAL LP':
                        ws.write_number(r_idx + 1, c_idx, int(val), total_f)
                    elif col_name in ['MÉDIA LP', 'MÉDIA CP', 'META'] or col_name in col_meses:
                        ws.write_number(r_idx + 1, c_idx, int(val), b_f_num)
                    else:
                        ws.write_string(r_idx + 1, c_idx, str(val), b_f_texto)

        st.download_button("📥 BAIXAR MATRIZ STAR (V136 - RESTAURADA)", output.getvalue(), "Matriz_STAR_Giri_V136.xlsx")
