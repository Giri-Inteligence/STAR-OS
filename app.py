...
if st.session_state.pagina_ativa == 'Matriz':
    ...
    if up:
        ...
        if dims_sel:
            ...
            total_venda = df_ag['TOTAL_ACUMULADO'].sum()
            df_ag['CURVA'] = (df_ag['TOTAL_ACUMULADO'].cumsum() / total_venda).apply(lambda x: 'A' if x <= 0.8 else ('B' if x <= 0.95 else 'C'))
            
            res = df_ag.apply(lambda r: engine_star(r, r['MEDIA_LP'], r['MEDIA_CP']), axis=1)
            df_ag['STATUS'], df_ag['META'], df_ag['AÇÃO'] = zip(*res)
            
            # --- INFOGRÁFICOS DE TRAÇÃO (Recuperados) ---
            # ... [Código dos Infográficos omitido aqui para focar na correção do Excel] ...

            # --- DRILL-DOWN E EXPORTAÇÃO ---
            st.markdown('<div style="font-size:0.85rem;font-weight:700;color:#ccc;margin-top:50px;margin-bottom:10px;">🔬 DRILL-DOWN TÁTICO: ISOLAMENTO DE CARTEIRA</div>', unsafe_allow_html=True)
            ...
            with c_ex:
                ...
                with pd.ExcelWriter(out, engine='xlsxwriter') as wr:
                    ...
                    # 1. CABEÇALHO (Azul Escuro, Letra Branca, Negrito, Bordas Brancas, Quebra de Texto)
                    h_fmt = wb.add_format({'bold':True, 'font_color':'#FFFFFF', 'bg_color':'#002060', 'border':1, 'border_color':'#FFFFFF', 'align':'center', 'valign':'vcenter', 'text_wrap':True})
                    
                    # 2. CORPO BASE (Alinhamento Vertical)
                    b_fmt = wb.add_format({'valign':'vcenter', 'align':'left', 'border':1, 'border_color':'#D9D9D9', 'text_wrap':True})
                    n_fmt = wb.add_format({'num_format':'#,##0', 'valign':'vcenter', 'align':'center', 'border':1, 'border_color':'#D9D9D9'})
                    bold_lbl = wb.add_format({'bold':True, 'valign':'vcenter', 'text_wrap':True})
                    
                    # 3. ESTILOS DE STATUS (Cores Específicas)
                    f_q = wb.add_format({'font_color':'#FF0000', 'bold':1, 'valign':'vcenter', 'border':1, 'border_color':'#D9D9D9', 'align':'left'})
                    f_q_ac = wb.add_format({'bg_color':'#FFC7CE', 'font_color':'#9C0006', 'bold':1, 'valign':'vcenter', 'border':1, 'border_color':'#D9D9D9', 'align':'left'})
                    f_c = wb.add_format({'bg_color':'#C6EFCE', 'font_color':'#006100', 'bold':1, 'valign':'vcenter', 'border':1, 'border_color':'#D9D9D9', 'align':'left'})
                    f_e = wb.add_format({'bg_color':'#DDEBF7', 'font_color':'#0070C0', 'bold':1, 'valign':'vcenter', 'border':1, 'border_color':'#D9D9D9', 'align':'left'})

                    # Aplicar Títulos
                    for i, v in enumerate(cols_v): ws.write(0, i, v, h_fmt)
                    
                    st_idx = cols_v.index('STATUS')
                    ac_idx = cols_v.index('AÇÃO')
                    
                    for r_idx, (idx, row) in enumerate(df_exib.iterrows()):
                        s_val = str(row['STATUS'])
                        target_fmt = b_fmt
                        if "QUEDA ACENTUADA" in s_val: target_fmt = f_q_ac
                        elif "🔴 QUEDA" in s_val: target_fmt = f_q
                        elif "CRESCIMENTO" in s_val: target_fmt = f_c
                        elif "ESTÁVEL" in s_val: target_fmt = f_e
                        
                        for c_idx, c_name in enumerate(cols_v):
                            v = row[c_name]
                            if c_idx == st_idx: ws.write(r_idx+1, c_idx, v, target_fmt)
                            elif c_idx == ac_idx:
                                parts = str(v).split('\n')
                                rich = []
                                for p in parts:
                                ...
                                else: ws.write(r_idx+1, c_idx, v, b_fmt)
                            elif isinstance(v, (int, float)): ws.write(r_idx+1, c_idx, int(v), n_fmt)
                            else: ws.write(r_idx+1, c_idx, v, b_fmt)
                    
                    ws.set_column(0, 0, 10) # Curva
                    ws.set_column(1, 1, 50) # Empresa (Largo)
                    ws.set_column(ac_idx, ac_idx, 75) # Ação (Muito Largo)
                    ws.set_default_row(70) # Altura para comportar o Método STAR
