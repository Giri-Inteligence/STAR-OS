...
# --- TELA 3: DESEMPENHO (INTACTA) ---
    ...
    with st.sidebar:
        ...
        vendedor_sel = st.selectbox("CONSULTOR:", [v.strip().upper() for v in nomes.split('\n') if v.strip()])
    st.markdown('<div class="title-center">MATRIZ DE DESEMPENHO</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="vendedor-destaque">{vendedor_sel}</div>', unsafe_allow_html=True)
    ...
    for i, col in enumerate(cols_in):
        with col:
            st.text_input(f"Indicador {i+1}", value=sugestoes[i], key=f"n_{i}_{vendedor_sel}")
            m_raw = st.text_input(f"Meta", value="", key=f"m_{i}_{vendedor_sel}")
            r_raw = st.text_input(f"Realizado", value="", key=f"r_{i}_{vendedor_sel}")
