        ...
        if dims_sel:
            ...
            with c_ex:
                ...
                with pd.ExcelWriter(out, engine='xlsxwriter') as wr:
                    ...
                    wrp_fmt = wb.add_format({'text_wrap':1,'valign':'vcenter','align':'left','border':1,'border_color':'#D9D9D9'})
                    num_fmt = wb.add_format({'num_format':'#,##0','valign':'vcenter','align':'center','border':1})
                    f_q_ac = wb.add_format({'bg_color':'#FFC7CE','font_color':'#9C0006','bold':1,'valign':'vcenter','border':1})
                    f_c_st = wb.add_format({'bg_color':'#C6EFCE','font_color':'#006100','bold':1,'valign':'vcenter','border':1})
                    f_e_st = wb.add_format({'bg_color':'#DDEBF7','font_color':'#0070C0', 'bold':1,'valign':'vcenter','border':1})
                    ...
                    for r_idx, (idx, row) in enumerate(df_exib.iterrows()):
                        ...
                        elif "CRESCIMENTO" in s_val: target_fmt = f_c_st
                        elif "ESTÁVEL" in s_val: target_fmt = f_e_st
                        
                        for c_idx, c_name in enumerate(cols_v):
                            v = row[c_name]
                            if c_idx == cols_v.index('STATUS'): ws.write(r_idx+1, c_idx, v, target_fmt)
                            elif isinstance(v, (int, float)): ws.write(r_idx+1, c_idx, int(v), num_fmt)
                            else: ws.write(r_idx+1, c_idx, v, wrp_fmt)
                    ws.set_column(0,0,8); ws.set_column(1,1,45); ws.set_column(len(cols_v)-1, len(cols_v)-1, 75); ws.set_default_row(65)
