...
if uploaded_file:
    ...
    with col_fat:
        ...
        for i, mes in enumerate(last3):
            fat = df_a[mes].sum()
            var_pct = None if i == 0 else ((fat - df_a[last3[i-1]].sum()) / df_a[last3[i-1]].sum() * 100 if df_a[last3[i-1]].sum() > 0 else None)
    ...
    st.markdown(f"""<div class="ana-wrap">
        ...
        <table class="ana-table">
            <thead><tr><th>CLASSIFICACAO</th><th style="text-align:left">CRITERIO</th><th>CLIENTES</th><th>%</th></tr></thead>
    ...
    for _, row in df_disp.iterrows():
        ...
        for col_n in cols_display:
            ...
            if col_n == 'STATUS':
                ...
                cells += f'<td{align_class}>{htmllib.escape(str(val))}</td>'
        rows_cart += f"<tr>{cells}</tr>"

    st.markdown(f"""<div class="cart-wrap">
