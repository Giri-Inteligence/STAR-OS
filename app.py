...
.section-title {
    font-size:0.95rem; font-weight:800; text-transform:uppercase; letter-spacing:2px;
    color:#1A2540; margin:32px 0 14px 0; padding-bottom:8px; border-bottom:2px solid #D1D9E6;
}
...
.briefing-wrap {
    ...
    box-shadow:0 4px 24px rgba(0,0,0,0.10); border-left:6px solid #001845;
}
...
.missao-card {
    ...
    box-shadow:0 4px 24px rgba(0,0,0,0.10); height:100%;
}
...
.step-num {
    background:#001845; color:#FFFFFF; border-radius:50%;
...
if uploaded_file:
    ...
    if score >= 5:
        sit_label, sit_badge, sit_cor_borda = "CRITICO", "badge-critico", "#C00000"
    elif score >= 2:
        sit_label, sit_badge, sit_cor_borda = "ATENCAO", "badge-atencao", "#B09000"
    else:
        sit_label, sit_badge, sit_cor_borda = "ESTAVEL", "badge-estavel", "#375623"

    # Montar texto do briefing
    frases = []
    if n_qa_a > 0:
        frases.append(f"{n_qa_a} cliente(s) Curva A em queda acentuada, representando receita mensal media de R$ {fmt_br(df_a[df_a['STATUS']=='QUEDA ACENTUADA']['MEDIA LP'].sum())}.")
    if n_q_a > 0:
        frases.append(f"{n_q_a} cliente(s) Curva A em queda — compras abaixo da media historica.")
    if abs(var_rec_a) > 5:
        direcao = "caiu" if var_rec_a < 0 else "cresceu"
        frases.append(f"A receita da Curva A {direcao} {abs(var_rec_a):.0f}% em relacao ao mes anterior ({penultimo} para {ultimo_mes}).")
    if n_in_a > 0:
        frases.append(f"{n_in_a} cliente(s) Curva A sem compra ha mais de 90 dias. Recuperacao urgente necessaria.")
    if fat_trend and len(last3) >= 3:
        frases.append(f"Faturamento Curva A em queda consecutiva nos 3 ultimos meses. Tendencia negativa estrutural.")
    if idx_saude < 50:
        frases.append(f"Apenas {idx_saude:.0f}% dos clientes Curva A estao em situacao saudavel (crescimento ou estavel).")
    if not frases:
        frases.append(f"Carteira Curva A com {idx_saude:.0f}% dos clientes em crescimento ou estaveis. Situacao sob controle.")

    texto_briefing = " ".join(frases)
    ...
    st.markdown(f"""
    <div class="briefing-wrap" style="border-left-color:{sit_cor_borda}">
        ...
        <div class="briefing-texto">{texto_briefing}</div>
    ...
    if n_qa_a > 0:
        ...
        missoes.append({
            'tipo': 'critica',
            'titulo': f'{n_qa_a} Cliente(s) Curva A em Queda Acentuada',
            'contexto': f'Esses clientes tinham media mensal de R$ {fmt_br(val_risco)} e estao comprando significativamente abaixo do historico. Risco real de perda.',
            'passos': [
                ('Prepare-se antes de ligar', 'Abra o historico de cada cliente. Veja o que ele comprava, quando parou e quanto valia por mes. Chegue ao contato com esses numeros em maos.'),
                ('Entre em contato — sem ofertar produto', 'O objetivo da primeira conversa e entender, nao vender. Perguntas: O que mudou na empresa? O que parou de precisar? Houve algum problema de entrega ou atendimento?'),
                ('Registre e agende o proximo passo', 'Anote o motivo identificado e defina a proxima acao com data. Traga essa informacao para a reuniao com o gestor.')
            ],
            'key': 'missao_qa'
        })
    ...
    if n_in_a > 0:
        missoes.append({
            'tipo': 'critica',
            'titulo': f'{n_in_a} Cliente(s) Curva A Inativo(s) — Mais de 90 Dias',
            'contexto': f'Clientes que representavam receita relevante e nao compram ha mais de 3 meses. Cada mes sem contato reduz a chance de reconquista.',
            'passos': [
                ('Revise o ultimo pedido de cada cliente', 'Qual foi o ultimo produto comprado? Quando foi? Qual era o valor? Esse contexto e essencial para a abordagem.'),
                ('Faca contato de diagnostico — sem pressao', 'Nao ligue para vender. Ligue para entender. Diga que notou a ausencia e quer saber como o cliente esta. Escute mais do que fala.'),
                ('Defina se ha potencial de reconquista', 'Com base na conversa, classifique: cliente recuperavel, cliente perdido, ou cliente a monitorar. Registre e agende proximo passo com data.')
            ],
            'key': 'missao_in'
        })
    ...
    if n_q_a > 0 and len(missoes) < 3:
        missoes.append({
            'tipo': 'urgente',
            'titulo': f'{n_q_a} Cliente(s) Curva A em Queda',
            'contexto': f'Compras abaixo da media historica mas ainda ativas. Janela de intervencao aberta — agir agora evita que virem queda acentuada.',
            'passos': [
                ('Compare o mix atual com o historico', 'Quais produtos reduziram? O cliente parou de comprar alguma categoria? Essa informacao orienta a conversa.'),
                ('Contato de manutencao com pergunta de expansao', 'Mantenha o relacionamento. Explore uma necessidade nova: existe algum produto que ele compra de outro fornecedor que voce poderia atender?'),
                ('Monitore o proximo mes', 'Se o cliente voltar a crescer, mantenha frequencia. Se cair mais, escale para protocolo de queda acentuada.')
            ],
            'key': 'missao_q'
        })
    ...
    if not missoes:
        missoes.append({
            'tipo': 'atencao',
            'titulo': 'Manter e Expandir a Curva A',
            'contexto': f'A carteira Curva A esta em situacao saudavel com {idx_saude:.0f}% dos clientes em crescimento ou estaveis. O foco agora e proteger e ampliar.',
            'passos': [
                ('Mantenha frequencia de contato com os clientes A', 'Clientes em crescimento sao os mais visados pela concorrencia. Esteja presente antes de precisar.'),
                ('Explore expansao de mix nos clientes estaveis', 'Identifique categorias que o cliente nao compra. Apresente uma nova opcao por contato.'),
                ('Monitore os indicadores mensalmente', 'Qualquer sinal de queda no ticket medio ou recencia deve virar acao imediata.')
            ],
            'key': 'missao_ok'
        })

    # Renderizar cards
    num_missoes = len(missoes)
    card_cols = st.columns(num_missoes) if num_missoes > 1 else [st.container()]

    for idx, (missao, col) in enumerate(zip(missoes, card_cols)):
        with col:
            tipo  = missao['tipo']
            steps_html = ""
            for si, (st_titulo, st_desc) in enumerate(missao['passos']):
                steps_html += f"""
                <div class="missao-step">
                    <div class="step-num {tipo}">{si+1}</div>
                    <div class="step-content">
                        <div class="step-titulo">{st_titulo}</div>
                        <div class="step-desc">{st_desc}</div>
                    </div>
                </div>"""

            st.markdown(f"""
            <div class="missao-card {tipo}">
                ...
                <div class="missao-footer">
                    <div class="missao-footer-titulo">Registrar proximo passo</div>
                </div>
            </div>""", unsafe_allow_html=True)

            chave = missao['key']
            st.text_input("Proximo passo:", placeholder="Descreva a acao a ser tomada...",
                         key=f"ps_{chave}_{idx}", label_visibility="collapsed")
            st.date_input("Data:", value=date.today(), key=f"dt_{chave}_{idx}",
    ...
    with cf:
        ...
        st.markdown(f"""<div class="ana-wrap"><div class="ana-title">FATURAMENTO CURVA A &mdash; ULTIMOS 3 MESES</div>
    ...
    with ct:
        ...
        st.markdown(f"""<div class="ana-wrap"><div class="ana-title">TICKET MEDIO CURVA A &mdash; ULTIMOS 3 MESES</div>
    ...
    l1  = f"Ultimo pedido em {meses_col[-2]}" if len(meses_col)>1 else "Ultimo pedido ha 1 mes"
    l2  = f"Ultimo pedido em {meses_col[-3]}" if len(meses_col)>2 else "Ultimo pedido ha 2 meses"
    l3p = f"Sem compra desde {meses_col[-4]} ou antes" if len(meses_col)>3 else "Sem compra ha 3+ meses"
    ...
    st.markdown(f"""<div class="ana-wrap"><div class="ana-title">RECENCIA DE COMPRA &mdash; CURVA A</div>
    ...
    with g1:
        ...
        lb=[s for s in STATUS_ORDER if s in sc.index]; vl=[sc[s] for s in lb]
        co=[STATUS_COLORS[s] for s in lb]; pc=[v/total*100 if total>0 else 0 for v in vl]
        ...
        fig1.update_layout(margin=dict(l=0,r=90,t=10,b=10),paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)',
    ...
    with g2:
        cc2=df['CURVA'].value_counts(); cvl=['A','B','C']
        cvv=[cc2.get(c,0) for c in cvl]; cvc=['#001845','#0056b3','#4A90C4']
        fig2=go.Figure(go.Pie(labels=cvl,values=cvv,hole=0.58,
            marker=dict(colors=cvc,line=dict(color='#FFFFFF',width=2)),
    ...
    if sel_vend=="Todos":
        ...
        for v in sorted(df[vend_col].dropna().astype(str).unique()):
            ...
            rv.append({'VENDEDOR':v,'CLIENTES':str(len(dv)),'CURVA A':str(len(dv[dv['CURVA']=='A'])),
        ...
        st.markdown(f"""<div class="vend-wrap"><table class="vend-table">
            <thead><tr>{hh}</tr></thead><tbody>{rh}</tbody></table></div>""", unsafe_allow_html=True)
    ...
    st.markdown(f"""<div class="cart-wrap"><table class="cart-table">
        <thead><tr>{hc}</tr></thead><tbody>{rc}</tbody></table></div>""", unsafe_allow_html=True)
